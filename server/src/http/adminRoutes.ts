...
function detail(id){var a=acctById(id),email=a?a.email:'';api('/admin/api/account?userId='+encodeURIComponent(id)).then(function(r){return r.json();}).then(function(d){var byId={};(d.statuses||[]).forEach(function(s){byId[s.deviceId]=s;});
  DDEVS=(d.devices||[]).map(function(dv){dv.email=email;return dv;});
  var rows=DDEVS.map(function(dv){var s=byId[dv.id]||{};return '<tr><td>'+esc(dv.name)+'</td><td class="muted">'+esc(dv.host)+'</td><td>'+esc(dv.firmware)+'</td><td>'+(s.state==='online'?'<span class="pill on">أونلاين</span>':'<span class="pill off">'+esc(s.state||'—')+'</span>')+'</td><td>'+(s.hashrateTHs?n1(s.hashrateTHs)+' TH':'—')+'</td><td>'+(s.maxTempC?Math.round(s.maxTempC)+'°':'—')+'</td><td class="muted">'+esc(s.worker||'—')+'</td><td><button class="btn sm" style="border-color:var(--acc);color:var(--acc)" onclick="flashDevice(\\''+dv.id+'\\')" title="فلاش فِرموير لهذا الجهاز">🔩</button></td></tr>';}).join('');
  document.getElementById('detail').innerHTML='<div class="panel"><div class="row" style="justify-content:space-between"><h2 style="margin:0">📋 '+esc(email)+' — '+(d.sites||[]).length+' موقع · '+(d.devices||[]).length+' جهاز</h2><button class="btn sm" onclick="document.getElementById(\\'detail\\').innerHTML=\\'\\'">إغلاق ✕</button></div><div class="scroll"><table><thead><tr><th>الجهاز</th><th>العنوان</th><th>الفرمور</th><th>الحالة</th><th>الهاش</th><th>الحرارة</th><th>الوركر</th><th>فلاش</th></tr></thead><tbody>'+(rows||'<tr><td colspan="8" class="muted">لا أجهزة</td></tr>')+'</tbody></table></div></div>';
...
function flashDevice(deviceId){var dev=null;var pool=DEVS.concat(DDEVS);for(var i=0;i<pool.length;i++){if(pool[i].id===deviceId){dev=pool[i];break;}}
  if(!dev){alert('افتح تفاصيل الحساب أو ابحث عن الجهاز أولاً ثم افلِشه');return;}
